...
### 王哲 — SST 距平时空分析
- `成员-王哲/A/static/index.html` + `static/js/main.js`
- ECharts 5.5 四图联动仪表盘：折线趋势 / 地理热力图 / OHC 柱状对比 / SST-风应力散点
- 联动机制：时间轴播放 + brush 刷选 + 地图点击下钻 + 散点 brush 高亮
...
- **数据文件改名**：如果修改 `data/` 下的 JSON 文件名，必须同步更新两处硬编码路径：`backend/data_adapter.py:27`（`_DATA_FILE`）和 `backend/app.py:132`（`/api/data/raw` 端点）。成员目录下的旧 `data_adapter.py`/`api.py` 已不被统一后端使用，可忽略。
